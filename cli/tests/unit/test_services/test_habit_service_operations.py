...
@pytest.fixture
def test_habit(session: Session, test_routine: Routine) -> Habit:
...
class TestUpdateHabit:
    ...
    def test_update_habit_title(self, session: Session, test_habit: Habit) -> None:
        """Atualiza título."""
        habit_service = HabitService(session)
        updated = habit_service.update_habit(test_habit.id, title="Novo Título")
        assert updated is not None
        assert updated.title == "Novo Título"

    def test_update_habit_times(self, session: Session, test_habit: Habit) -> None:
        """Atualiza horários."""
        habit_service = HabitService(session)
    ...
    def test_update_habit_recurrence(self, session: Session, test_habit: Habit) -> None:
    ...
    def test_update_habit_color(self, session: Session, test_habit: Habit) -> None:
        """Atualiza cor."""
        habit_service = HabitService(session)
        updated = habit_service.update_habit(test_habit.id, color="#FF5733")
        assert updated.color == "#FF5733"

    def test_update_habit_multiple_fields(self, session: Session, test_habit: Habit) -> None:
    ...
    def test_update_habit_not_found(self, session: Session) -> None:
        """Retorna None para ID inexistente."""
        habit_service = HabitService(session)
        assert habit_service.update_habit(9999, title="Teste") is None

    def test_update_habit_with_empty_title(self, session: Session, test_habit: Habit) -> None:
        """Rejeita título vazio."""
        habit_service = HabitService(session)
        with pytest.raises(ValueError, match="cannot be empty"):
            habit_service.update_habit(test_habit.id, title="   ")

    def test_update_habit_with_title_too_long(self, session: Session, test_habit: Habit) -> None:
        """Rejeita título muito longo."""
        habit_service = HabitService(session)
        with pytest.raises(ValueError, match="cannot exceed 200"):
            habit_service.update_habit(test_habit.id, title="X" * 201)

    def test_update_habit_with_invalid_times(self, session: Session, test_habit: Habit) -> None:
        """Rejeita start >= end."""
        habit_service = HabitService(session)
...
class TestDeleteHabit:
    ...
    def test_delete_habit_not_found(self, session: Session) -> None:
        ...
        habit_service = HabitService(session)
        assert habit_service.delete_habit(9999) is False
