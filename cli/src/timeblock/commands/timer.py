    ...
    with Live(refresh_per_second=1, console=console) as live:
        while True:
            try:
                ...
                console.print("\n\nTimer ainda ativo. Use 'timeblock timer stop' ou 'timer cancel'", style="yellow")
                return
...
def start_timer(
    ...
    try:
        ...
        if active:
            console.print("✗ Já existe um timer ativo. Use 'timer stop' ou 'timer cancel' primeiro.", style="red")
            raise typer.Exit(1)
        
        # Workflow A: direto com flags
        if schedule or task:
            target_id = schedule or task
            target_type = "schedule" if schedule else "task"
        ...
        # Workflow B: via select
            ...
            if not selected:
                console.print("✗ Nenhuma instância selecionada. Use 'timeblock schedule select <id>' ou '--schedule <id>'", style="red")
                raise typer.Exit(1)
