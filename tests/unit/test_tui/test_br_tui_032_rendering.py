...
class TestBRTUI032:
    ...
    def test_br_tui_032_block_first_line_title_icon(
        ...
        """BR-TUI-032-R4: Primeira linha tem accent bar, título em C_TEXT e ícone."""
        ...
        # Deve conter accent bar ▌
        assert "\u258c" in first_line, "Accent bar ausente na linha de título"
    ...
    def test_br_tui_032_block_body_accent_bar_color(
        ...
        """BR-TUI-032-R5: Linhas de corpo têm accent bar + fill na cor do status."""
        instances = [block_10_11]
        _, blocks, _ = build_agenda_content(instances, now=now)
        ...
        for line_num in [41, 42, 43]:
            ...
            assert "\u258c" in body_line, f"Accent bar ausente na linha {line_num}"
    ...
    def test_br_tui_032_no_horizontal_lines_through_blocks(
        ...
        """BR-TUI-032-R9: Nenhuma linha horizontal atravessa um bloco."""
        instances = [block_10_11]
        _, blocks, _ = build_agenda_content(instances, now=now)
    ...
    def test_br_tui_032_end_time_line_has_color(
        ...
        """BR-TUI-032-R10: Linha do horário de término ainda exibe cor."""
        instances = [block_10_11]
        _, blocks, _ = build_agenda_content(instances, now=now)

        # 11:00 = 660min / 15 = line 44 (R10: end line com cor)
        idx = _line_index(44, instances)
        end_line = blocks[idx]

        assert "\u258c" in end_line, "Accent bar ausente na end line"
        assert "\u2591" in end_line, "Fill char ausente na end line (R10)"
    ...
    def test_br_tui_032_consecutive_blocks_no_gap(
        ...
        """BR-TUI-032-R12: Bloco consecutivo substitui corpo do anterior."""
    ...
    def test_br_tui_032_minimum_column_width_18(
        ...
        """BR-TUI-032-R13: Largura mínima por coluna é 18 caracteres."""
    ...
    def test_br_tui_032_empty_area_is_empty(self, block_10_11: dict, now: datetime):
        """BR-TUI-032-R17: Áreas sem bloco são string vazia."""
        instances = [block_10_11]
        _, blocks, _ = build_agenda_content(instances, now=now)
