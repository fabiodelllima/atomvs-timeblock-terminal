...
class TestBRTUI003DashboardScreen:
    ...
    async def test_br_tui_003_has_two_columns(self):
        """Dashboard possui duas colunas: agenda (~55%) e cards (~45%)."""
        async with TimeBlockApp().run_test() as pilot:
            await pilot.press("1")
            agenda_col = pilot.app.query("#agenda-column")
            cards_col = pilot.app.query("#panels-column")
            assert len(agenda_col) == 1
            assert len(cards_col) == 1
    ...
    async def test_br_tui_003_shows_pending_tasks(self):
        ...
        async with TimeBlockApp().run_test() as pilot:
            cards_col = pilot.app.query_one("#panels-column")
