# Regras de Negócio

**Versão:** 3.1.0

**Status:** Consolidado (SSOT)

---

## Conceitos Fundamentais

Os conceitos introdutórios e o glossário do domínio estão em [Introdução e Fundamentos](../../explanation/domain-concepts.md).

## Domínios

- [Routine](br-routine.md)
- [Habit](br-habit.md)
- [HabitInstance](br-habitinstance.md)
- [Skip](br-skip.md)
- [Streak](br-streak.md)
- [Task](br-task.md)
- [Timer](br-timer.md)
- [Event Reordering](br-event.md)
- [Validações Globais](br-validation.md)
- [CLI](br-cli.md)
- [Tag](br-tag.md)
- [TUI](br-tui.md)
- [Data](br-data.md)
- [Testing](br-testing.md)

---

## Referências

- **ADRs:** `docs/decisions/`
- **Livro:** "Atomic Habits" - James Clear
- **Service Layer:** `src/timeblock/services/`
- **Models:** `src/timeblock/models/`
- **Enums:** `src/timeblock/models/enums.py`

---

**Última atualização em:** 19 de Marco de 2026

**Total de regras:** 104 BRs
